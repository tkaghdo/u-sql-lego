...
// Explicit instantiations for templated functions that are defined in managed code
    ...
    public:
        int m_COLOR_ID;
        FString m_COLOR_NAME;
        FString m_COLOR_RGB;
        FString m_IS_TRANSPARENT;
        Extract_0_Data0();
    ...
    INLINE ostream & operator<<(ostream & os, Extract_0_Data0 & row)
    {
        ...
        os << "\t" << std::string{ u8"COLOR_NAME" } << ":\t" << row.m_COLOR_NAME << endl;
        os << "\t" << std::string{ u8"COLOR_RGB" } << ":\t" << row.m_COLOR_RGB << endl;
        os << "\t" << std::string{ u8"IS_TRANSPARENT" } << ":\t" << row.m_IS_TRANSPARENT << endl;
        return os;
    }
#endif // defined(SCOPE_DEBUG)
    ...
    struct ManagedRow<Extract_0_Data0>
    {
        cli::array<ScopeEngineManaged::ColumnOffsetId>^ ColumnOffsets()
        {
            ...
            offset[0] = ColumnOffsetId(offsetof(Extract_0_Data0, m_COLOR_ID), T_Integer);
            offset[1] = ColumnOffsetId(offsetof(Extract_0_Data0, m_COLOR_NAME), T_String);
            offset[2] = ColumnOffsetId(offsetof(Extract_0_Data0, m_COLOR_RGB), T_String);
            offset[3] = ColumnOffsetId(offsetof(Extract_0_Data0, m_IS_TRANSPARENT), T_String);
            return offset;        
        }
        ...
        cli::array<ScopeEngineManaged::SqlIpColumn^>^ Columns(cli::array<String^>^ readOnlyColumns)
        {
            ...
            c[0] = gcnew ScopeEngineManaged::SqlIpColumn(gcnew System::String("COLOR_ID"), System::Int32::typeid, readOnlyColumns == nullptr ? false : Array::IndexOf(readOnlyColumns, gcnew System::String("COLOR_ID")) >= 0, /*isSystem*/false);
            c[1] = gcnew ScopeEngineManaged::SqlIpColumn(gcnew System::String("COLOR_NAME"), System::String::typeid, readOnlyColumns == nullptr ? false : Array::IndexOf(readOnlyColumns, gcnew System::String("COLOR_NAME")) >= 0, /*isSystem*/false);
            c[2] = gcnew ScopeEngineManaged::SqlIpColumn(gcnew System::String("COLOR_RGB"), System::String::typeid, readOnlyColumns == nullptr ? false : Array::IndexOf(readOnlyColumns, gcnew System::String("COLOR_RGB")) >= 0, /*isSystem*/false);
            c[3] = gcnew ScopeEngineManaged::SqlIpColumn(gcnew System::String("IS_TRANSPARENT"), System::String::typeid, readOnlyColumns == nullptr ? false : Array::IndexOf(readOnlyColumns, gcnew System::String("IS_TRANSPARENT")) >= 0, /*isSystem*/false);
            return c;        
        }
...
// project, and then turn off /doc.
    ...
    INLINE Extract_0_Data0::Extract_0_Data0(const Extract_0_Data0 & c, IncrementalAllocator * alloc) :
            m_COLOR_NAME(c.m_COLOR_NAME, alloc),
            m_COLOR_RGB(c.m_COLOR_RGB, alloc),
            m_IS_TRANSPARENT(c.m_IS_TRANSPARENT, alloc)
    {
        ...
        static bool Deserialize(TextInputStream<TextInputStreamTraitsConst<',', '\0', '\0', false, true, UTF8, true, false, false, 1, CharFormat_uint16>, CosmosInput> * input, Extract_0_Data0 & row)
        {
            for(;;)
            {
                try
                {
                    ...
                    input->Read(row.m_COLOR_NAME);
                    input->Read(row.m_COLOR_RGB);
                    input->Read(row.m_IS_TRANSPARENT);
...
// project, and then turn off /doc.
    ...
    public:
        ...
        static bool FilterTransformRow(Extract_0_Data0 & input, Extract_0_Data0 & output, IncrementalAllocator * alloc)
        {
           ...
           output.m_COLOR_RGB = input.m_COLOR_RGB;
            return true;
        }
...
// project, and then turn off /doc.
    ...
    public:
        ...
        {
            ScopeManagedInterop::CopyToNativeColumn(nativeRow.m_COLOR_NAME, managedRow->GetInternal<System::String^>(1), alloc);
            ScopeManagedInterop::CopyToNativeColumn(nativeRow.m_IS_TRANSPARENT, managedRow->GetInternal<System::String^>(3), alloc);
        }
    };
#endif // defined(COMPILE_MANAGED)
// All of the templates will throw a CS1591 warning because we 
...
// project, and then turn off /doc.
    ...
    public:
        ...
        static void Serialize(TextOutputStream<TextOutputStreamTraitsConst<',', '\0', false, false, true, false, true, UTF8, true, CharFormat_uint16>, CosmosOutput> * output, Extract_0_Data0 & row)
        {
            ...
            if (!row.m_COLOR_NAME.IsNull())
            {
                output->Write<FString,true>(row.m_COLOR_NAME);
            }
            output->WriteDelimiter();
            if (!row.m_COLOR_RGB.IsNull())
            {
                output->Write<FString,true>(row.m_COLOR_RGB);
            }
            output->WriteDelimiter();
            if (!row.m_IS_TRANSPARENT.IsNull())
            {
                output->Write<FString,true>(row.m_IS_TRANSPARENT);
            }
            output->WriteNewLine();
        }
        ...
        static void SerializeHeader(TextOutputStream<TextOutputStreamTraitsConst<',', '\0', false, false, true, false, true, UTF8, true, CharFormat_uint16>, CosmosOutput> * output)
        {
            ...
            output->WriteDelimiter();
            output->Write<string,true>(FString("COLOR_NAME", 10));
            output->WriteDelimiter();
            output->Write<string,true>(FString("COLOR_RGB", 9));
            output->WriteDelimiter();
            output->Write<string,true>(FString("IS_TRANSPARENT", 14));
            output->WriteNewLine();
        }
    };
