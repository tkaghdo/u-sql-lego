...
// Explicit instantiations for templated functions that are defined in managed code
    ...
    INLINE ostream & operator<<(ostream & os, Extract_0_Data0 & row)
    {
        ...
        os << "\t" << std::string{ u8"PART_NAME" } << ":\t" << row.m_PART_NAME << endl;
        os << "\t" << std::string{ u8"CATEGORY_ID" } << ":\t" << row.m_CATEGORY_ID << endl;
        return os;
    }
#endif // defined(SCOPE_DEBUG)
    ...
    struct ManagedRow<Extract_0_Data0>
    {
        cli::array<ScopeEngineManaged::ColumnOffsetId>^ ColumnOffsets()
        {
            ...
            offset[0] = ColumnOffsetId(offsetof(Extract_0_Data0, m_PART_NUMBER), T_String);
            offset[1] = ColumnOffsetId(offsetof(Extract_0_Data0, m_PART_NAME), T_String);
            offset[2] = ColumnOffsetId(offsetof(Extract_0_Data0, m_CATEGORY_ID), T_Integer);
            return offset;        
        }
        ...
        cli::array<ScopeEngineManaged::SqlIpColumn^>^ Columns(cli::array<String^>^ readOnlyColumns)
        {
            ...
            c[0] = gcnew ScopeEngineManaged::SqlIpColumn(gcnew System::String("PART_NUMBER"), System::String::typeid, readOnlyColumns == nullptr ? false : Array::IndexOf(readOnlyColumns, gcnew System::String("PART_NUMBER")) >= 0, /*isSystem*/false);
            c[1] = gcnew ScopeEngineManaged::SqlIpColumn(gcnew System::String("PART_NAME"), System::String::typeid, readOnlyColumns == nullptr ? false : Array::IndexOf(readOnlyColumns, gcnew System::String("PART_NAME")) >= 0, /*isSystem*/false);
            c[2] = gcnew ScopeEngineManaged::SqlIpColumn(gcnew System::String("CATEGORY_ID"), System::Int32::typeid, readOnlyColumns == nullptr ? false : Array::IndexOf(readOnlyColumns, gcnew System::String("CATEGORY_ID")) >= 0, /*isSystem*/false);
            return c;        
        }
...
// project, and then turn off /doc.
    ...
    INLINE Extract_0_Data0::Extract_0_Data0(const Extract_0_Data0 & c, IncrementalAllocator * alloc) :
            ...
            m_PART_NAME(c.m_PART_NAME, alloc)
    {
        m_CATEGORY_ID = c.m_CATEGORY_ID;
    }
        ...
        static bool Deserialize(TextInputStream<TextInputStreamTraitsConst<',', '\0', '\0', false, true, UTF8, true, false, false, 1, CharFormat_uint16>, CosmosInput> * input, Extract_0_Data0 & row)
        {
            for(;;)
            {
                try
                {
                    ...
                    input->Read(row.m_PART_NAME);
                    input->Read(row.m_CATEGORY_ID);
...
// project, and then turn off /doc.
    ...
    public:
        ...
        static bool FilterTransformRow(Extract_0_Data0 & input, Extract_0_Data0 & output, IncrementalAllocator * alloc)
        {
           ...
           output.m_CATEGORY_ID = input.m_CATEGORY_ID;
            return true;
        }
...
// project, and then turn off /doc.
    ...
    public:
        ...
        {
            ScopeManagedInterop::CopyToNativeColumn(nativeRow.m_PART_NAME, managedRow->GetInternal<System::String^>(1), alloc);
        }
    };
#endif // defined(COMPILE_MANAGED)
// All of the templates will throw a CS1591 warning because we 
...
// project, and then turn off /doc.
    ...
    public:
        ...
        static void Serialize(TextOutputStream<TextOutputStreamTraitsConst<',', '\0', false, false, true, false, true, UTF8, true, CharFormat_uint16>, CosmosOutput> * output, Extract_0_Data0 & row)
        {
            ...
            if (!row.m_PART_NAME.IsNull())
            {
                output->Write<FString,true>(row.m_PART_NAME);
            }
            output->WriteDelimiter();
            output->Write<int,false>(row.m_CATEGORY_ID);
            output->WriteNewLine();
        }
        ...
        static void SerializeHeader(TextOutputStream<TextOutputStreamTraitsConst<',', '\0', false, false, true, false, true, UTF8, true, CharFormat_uint16>, CosmosOutput> * output)
        {
            ...
            output->WriteDelimiter();
            output->Write<string,true>(FString("PART_NAME", 9));
            output->WriteDelimiter();
            output->Write<string,true>(FString("CATEGORY_ID", 11));
            output->WriteNewLine();
        }
    };
