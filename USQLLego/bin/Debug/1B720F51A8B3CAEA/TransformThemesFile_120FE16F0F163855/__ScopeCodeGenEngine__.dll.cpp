...
// Explicit instantiations for templated functions that are defined in managed code
    ...
    public:
        int m_THEME_ID;
        FString m_THEME_NAME;
        NativeNullable<int> m_THEME_PARENT_ID;
        Extract_0_Data0();
    ...
    {
        os << "\t" << std::string{ u8"THEME_ID" } << ":\t" << row.m_THEME_ID << endl;
        os << "\t" << std::string{ u8"THEME_NAME" } << ":\t" << row.m_THEME_NAME << endl;
        os << "\t" << std::string{ u8"THEME_PARENT_ID" } << ":\t" << row.m_THEME_PARENT_ID << endl;
        return os;
    }
#endif // defined(SCOPE_DEBUG)
    ...
    struct ManagedRow<Extract_0_Data0>
    {
        cli::array<ScopeEngineManaged::ColumnOffsetId>^ ColumnOffsets()
        {
            ...
            offset[0] = ColumnOffsetId(offsetof(Extract_0_Data0, m_THEME_ID), T_Integer);
            offset[1] = ColumnOffsetId(offsetof(Extract_0_Data0, m_THEME_NAME), T_String);
            offset[2] = ColumnOffsetId(offsetof(Extract_0_Data0, m_THEME_PARENT_ID), T_IntegerQ);
            return offset;        
        }
        ...
        cli::array<ScopeEngineManaged::SqlIpColumn^>^ Columns(cli::array<String^>^ readOnlyColumns)
        {
            ...
            c[0] = gcnew ScopeEngineManaged::SqlIpColumn(gcnew System::String("THEME_ID"), System::Int32::typeid, readOnlyColumns == nullptr ? false : Array::IndexOf(readOnlyColumns, gcnew System::String("THEME_ID")) >= 0, /*isSystem*/false);
            c[1] = gcnew ScopeEngineManaged::SqlIpColumn(gcnew System::String("THEME_NAME"), System::String::typeid, readOnlyColumns == nullptr ? false : Array::IndexOf(readOnlyColumns, gcnew System::String("THEME_NAME")) >= 0, /*isSystem*/false);
            c[2] = gcnew ScopeEngineManaged::SqlIpColumn(gcnew System::String("THEME_PARENT_ID"), System::Nullable<System::Int32>::typeid, readOnlyColumns == nullptr ? false : Array::IndexOf(readOnlyColumns, gcnew System::String("THEME_PARENT_ID")) >= 0, /*isSystem*/false);
            return c;        
        }
...
// project, and then turn off /doc.
    ...
    INLINE Extract_0_Data0::Extract_0_Data0(const Extract_0_Data0 & c, IncrementalAllocator * alloc) :
            m_THEME_NAME(c.m_THEME_NAME, alloc),
            m_THEME_PARENT_ID(c.m_THEME_PARENT_ID)
    {
        m_THEME_ID = c.m_THEME_ID;
    }
        ...
        static bool Deserialize(TextInputStream<TextInputStreamTraitsConst<',', '\0', '\0', false, true, UTF8, true, false, false, 1, CharFormat_uint16>, CosmosInput> * input, Extract_0_Data0 & row)
        {
            for(;;)
            {
                try
                {
                    ...
                    input->Read(row.m_THEME_ID);
                    input->Read(row.m_THEME_NAME);
                    input->Read(row.m_THEME_PARENT_ID);
...
// project, and then turn off /doc.
    ...
    public:
        ...
        {
           output.m_THEME_ID = input.m_THEME_ID;
           output.m_THEME_PARENT_ID = input.m_THEME_PARENT_ID;
            return true;
        }
...
// project, and then turn off /doc.
    ...
    public:
        ...
        {
            ScopeManagedInterop::CopyToNativeColumn(nativeRow.m_THEME_NAME, managedRow->GetInternal<System::String^>(1), alloc);
        }
    };
#endif // defined(COMPILE_MANAGED)
// All of the templates will throw a CS1591 warning because we 
...
// project, and then turn off /doc.
    ...
    public:
        ...
        {
            output->Write<int,false>(row.m_THEME_ID);
            output->WriteDelimiter();
            if (!row.m_THEME_NAME.IsNull())
            {
                output->Write<FString,true>(row.m_THEME_NAME);
            }
            output->WriteDelimiter();
            if (!row.m_THEME_PARENT_ID.IsNull())
            {
                output->Write<int,false>(row.m_THEME_PARENT_ID);
            }
            output->WriteNewLine();
        }
        ...
        {
            output->Write<string,true>(FString("THEME_ID", 8));
            output->WriteDelimiter();
            output->Write<string,true>(FString("THEME_NAME", 10));
            output->WriteDelimiter();
            output->Write<string,true>(FString("THEME_PARENT_ID", 15));
            output->WriteNewLine();
        }
    };
